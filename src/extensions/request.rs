use crate::{constants, extensions::HeaderMapExtensions, uuid};
...
impl<T: std::fmt::Debug> RequestExtensions<T> for Request<T> {
    fn get_correlation_id(&self) -> HeaderValue {
        self.headers()
            ...
            .unwrap_or(&HeaderValue::from_str(uuid::get().as_str()).unwrap())
            .clone()
    }
    ...
    fn log(&self) {
        ...
        log::info!(
            "origin: {:?}",
            headers.get_or_default(ORIGIN.as_str(), "unknown")
        );
        log::info!(
            "traceparent: {:?}",
